...
export const getPages = cache(async (query?: GetContentsQuery): Promise<{ articles: Article[]; total: number }> => {
...
export const getPage = cache(async (slug: string): Promise<Article | null> => {
  if (!slug) return null;
